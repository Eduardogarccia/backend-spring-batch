...
import br.com.backend.models.Transacao;
import br.com.backend.models.TransacaoCNAB;

@Configuration
public class BatchConfig {
	
	private PlatformTransactionManager transactionManeger;
	
	private JobRepository jobRepository;

	public BatchConfig(PlatformTransactionManager transactionManeger, JobRepository jobRepository) {
		this.transactionManeger = transactionManeger;
	...
	Step step(ItemReader<TransacaoCNAB> reader, ItemProcessor<TransacaoCNAB, Transacao> processor, ItemWriter<Transacao> writer) {
		return  new StepBuilder("step", jobRepository)
				.<TransacaoCNAB, Transacao>chunk(1000, transactionManeger)
				.reader(reader)
				.processor(processor)
				.writer(writer)
				.build();
